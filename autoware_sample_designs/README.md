...
parameters:
  - node: /perception/object_recognition/detection/clustering/...
    param_files:
      ...
      - param_path: $(find-pkg-share autoware_perception_launch)/config/...
```
...
This invokes `autoware_system_designer_build_deploy()` from `CMakeLists.txt` once per `*.system.yaml`, and generates all launch files under `install/`.
...
In the visualization page, navigate to the deploy system, open the **Launch** section, and copy the command for the desired mode:
